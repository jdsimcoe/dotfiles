-- Terminal ANSI palette variant of the installed Mandelbrot visualizer for cliamp.
-- Colors follow the terminal theme; animation preserved from mandelbrot.lua.
...
local p = plugin.register({
    ...
    type = "visualizer",
    version = "1.1.0",
    description = "Mandelbrot using the terminal's normal and bright ANSI colors",
})
...
local PALETTES = {
    grayscale = {8, 7, 15},
    ansi = {1, 9, 5, 13, 4, 12, 6, 14, 2, 10, 3, 11},
}
local PAL = 30
...
local function set_palette(name)
    if name == "theme" then name = "ansi" end
    if not PALETTES[name] then return false end
    if name == active_palette then return true end

    -- Traverse each palette and reverse over the original 30 animation steps.
    local colors = PALETTES[name]
    local count = #colors
    for i = 1, PAL do
        local step = m_floor((i - 1) * (count * 2) / PAL) + 1
        local slot = colors[step <= count and step or (count * 2 + 1 - step)]
        local code = slot < 8 and (30 + slot) or (90 + slot - 8)
        local dc = ESC .. "[2;" .. code .. "m"
        local bc = ESC .. "[" .. code .. "m"
        local td = {}; local tb = {}
        for mask = 0, 255 do
            local br = string.char(0xE2, 0xA0 + m_floor(mask / 64), 0x80 + mask % 64)
            td[mask] = dc .. br .. RESET
            tb[mask] = bc .. br .. RESET
        end
        BRAILE[i] = td
...
function p:init()
    local saved = cliamp.store.get("palette")
    if not set_palette(saved or "ansi") then set_palette("ansi") end
    self.zoom          = MIN_ZOOM
    self.zoom_dir      = 1
    self.zoom_log      = 0.0
    self.cos_t         = 1.0
...
function p:render(bands, frame, rows, cols)
    ...
    -- cliamp v2.2.0 does not call visualizer init hooks before rendering.
...
-- Switch and persist the palette without restarting the player.
p:command("palette", function(args)
    local name = args[1]
    if not name then return active_palette or "ansi" end
    if not set_palette(name) then
        return "Usage: palette ansi|grayscale"
    end
    local ok, err = cliamp.store.set("palette", active_palette)
    if not ok then return "Palette applied but could not save: " .. tostring(err) end
    return "ANSIbrot palette: " .. active_palette
end)
